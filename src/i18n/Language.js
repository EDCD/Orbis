...
export function getLanguage(langCode) {
  ...
  const currentTerms = lang.terms;
  const d3Locale = d3.formatLocale(lang.formats);
  const gen = d3Locale.format('');
  ...
  if (lang === EN) {
    translate = (t, x) => currentTerms[`${t}_${x}`] || currentTerms[t] || t;
  } else {
    translate = (t, x) =>
      currentTerms[`${t}_${x}`] ||
      currentTerms[t] ||
  ...
  return {
    formats: {
      ...
      time: d =>
        `${(d < 0 ? '-' : '') + Math.floor(Math.abs(d) / 60)}:${`00${Math.floor(
          Math.abs(d) % 60
        )}`.substr(-2, 2)}`
    },
    translate,
